...
public class BookingTab {
	...
	public void initialize() {
		...
		bookTicket.setOnAction(
				(event) -> {
					...
					int value = db.bookShow(movie, date);
					if (value > 0){
						report("Booked one ticket to " + movie + " on " + date + " with reservation number: " + value);
						fillShow(movie, date);
					} else{
						report("Show could not be booked");
					}
				});
	...
	private void fillNamesList() {
		moviesList.setItems(FXCollections.observableList(db.movies()));
		// remove any selection
		moviesList.getSelectionModel().clearSelection();
	}

	private void fillDatesList(String m) {
		datesList.setItems(FXCollections.observableList(db.moviesDates(m)));
		// remove any selection
